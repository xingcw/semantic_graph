    ...
    def transfer_to_torch(self, data, training=True):
        ...
        if node_num_2 > self.args.node_num:
            ...
            data["nodes_2"] = np.concatenate(
                (np.array(data["nodes_2"]), -np.ones(self.args.node_num - node_num_2))).tolist()
    ...
    def fit(self):
        ...
        for epoch in epochs:
            ...
            for index, batch in tqdm(enumerate(batches), total=len(batches), desc="Batches"):
                ...
                self.writer.add_scalar('Train_sum', loss,
                                       int(epoch) * len(batches) * int(self.args.batch_size) + main_index)
                self.writer.add_scalar('Train loss', loss_score,
                                       int(epoch) * len(batches) * int(self.args.batch_size) + main_index)
    ...
    def eval_batch_pair(self, batch):
        ...
        def get_embeddings():
            def hook(model, inputs, output):
                activation["features"] = output[0].detach()
                activation["scores"] = output[1].detach()
            return hook

        self.model.module.attention.register_forward_hook(get_embeddings())
    ...
    def write_soft_label(self, data_dir):
        ...
        for i in range(len(eval_graphs)):
            ...
            file_name = os.path.join(
                "/media/work/data/kitti/odometry/semantic-kitti/DGCNN_graph_pairs_3_20/pred_label/05",
                pair_file.split('/')[-1])
